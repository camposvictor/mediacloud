...
from core.models import ImageFile, AudioFile, VideoFile
...
def dashboard(request):
    ...
    if media_type == 'image':
        media_files = ImageFile.objects.all()
    elif media_type == 'video':
        media_files = VideoFile.objects.all()
    elif media_type == 'audio':
        media_files = AudioFile.objects.all()
    else:
        # Consultas individuais para cada tipo de mídia
        image_files = list(ImageFile.objects.all())
        video_files = list(VideoFile.objects.all())
        audio_files = list(AudioFile.objects.all())

        # Combina todos os resultados em uma lista
        media_files = image_files + video_files + audio_files
