   ...
   def post(self, request):
    if request.FILES.get('file'):
        ...
        if uploaded_file.size > 5 * 1024 * 1024:
            return HttpResponse("<span id='upload-form-alert-audio' class='alert alert-error my-2'>O arquivo excede o tamanho máximo de 10MB", status=400)
        if uploaded_file.content_type.startswith('audio/'):
            media_type = 'audio'
        else:
            return HttpResponse("<span id='upload-form-alert-audio' class='alert alert-error my-2'>Tipo de arquivo não suportado.</span>", status=400)
